...
from dataset_big import ListDataset
...
def train():
    ...
    optimizer = torch.optim.SGD(net.parameters(), lr=learning_rate, momentum=0.99, weight_decay=0.00055)
    #optimizer = torch.optim.Adam(net.parameters(), lr=learning_rate, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[248, 300], gamma=0.1)
    ...
    for epoch in range(num_epochs):
        ...
        vis.line(Y=np.array([val_loss]), X=np.array([epoch*30+30]),
                 win=win,
                 name='val',
                 update='append')
